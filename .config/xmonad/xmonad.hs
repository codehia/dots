...
import XMonad.Hooks.StatusBar ( StatusBarConfig , dynamicEasySBs , statusBarPropTo, statusBarGeneric)
...
xmobarPrimary = statusBarPropTo "_XMONAD_LOG_0" "xmobar -x 0 ~/.config/xmobar/xmobarrc0.hs" (pure $ myXmobarPP 0)
xmobarSecondary = statusBarPropTo "_XMONAD_LOG_1" "xmobar -x 1 ~/.config/xmobar/xmobarrc1.hs" (pure $ myXmobarPP 1)
...
mytraySB :: StatusBarConfig
mytraySB = statusBarGeneric
            (unwords 
              [ "trayer" ,"--edge top" ,"--align right" ,"--distance 4,2" ,"--distancefrom top,right"
              , "--widthtype request"
              , "--transparent true", "--height 30", "--alpha 0" , "--tint 0x282c34"
              , "--padding 5", "--monitor 0" , "--iconspacing 3"
              , "--expand true" 
              ]) mempty

myStatusBarSpawner :: ScreenId -> IO StatusBarConfig
myStatusBarSpawner 0 = pure $ xmobarPrimary <> mytraySB -- two bars on the main screen
myStatusBarSpawner 1 = pure xmobarSecondary
myStatusBarSpawner _ = mempty -- nothing on the rest of the screens
